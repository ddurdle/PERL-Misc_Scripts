...
if ($arglist =~ m% dash %){
	...
	$arglist =~ s%\-i .* \-f segment \-segment_format matroska .* -segment_list %\-i "$video" \-codec\:v\:0 copy \-copyts \-vsync \-1 \-codec\:a\:0 copy \-copypriorss\:a\:0 0 -segment_format mpegts -f ssegment -individual_header_trailer 0 -segment_time 1 -segment_start_number 128 -segment_copyts 1 -segment_time_delta 0.0625 -segment_list %;
